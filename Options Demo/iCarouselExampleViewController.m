...
- (UIView *)carousel:(iCarousel *)carousel viewForItemAtIndex:(NSInteger)index reusingView:(UIView *)view
{
    ...
    if (view == nil)
    {
        view = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width - 44, 150)];
...
- (CGFloat)carousel:(iCarousel *)carousel valueForOption:(iCarouselOption)option withDefault:(CGFloat)value
{
    switch (option)
    {
        ...
        {
            NSLog(@"_arcSlider   %.02f",_arcSlider.value);
            return 2 * M_PI * _arcSlider.value;
        }
        case iCarouselOptionRadius:
        {
            NSLog(@" _radiusSlider     %.02f",_radiusSlider.value);
            return value * _radiusSlider.value;
        }
        case iCarouselOptionTilt:
        {
            NSLog(@"_tiltSlider    %.02f",_tiltSlider.value);
            return _tiltSlider.value;
        }
        case iCarouselOptionSpacing:
        {
            NSLog(@"_spacingSlider     %.02f",_spacingSlider.value);
